...
describe("Vercel build-time sandbox prewarm", () => {
  ...
  it("rejects Vercel output that requires sandbox templates without an OIDC token", async () => {
    ...
    vi.stubEnv("VERCEL_PROJECT_ID", "prj_without_credentials");
  ...
  it("skips prewarm for Vercel output analysis without deployment context", async () => {
    vi.stubEnv("VERCEL", "1");
    vi.stubEnv("VERCEL_OIDC_TOKEN", "");
  ...
  it("allows a deployment build without OIDC when no sandbox template is required", async () => {
    vi.stubEnv("VERCEL", "1");
    vi.stubEnv("VERCEL_OIDC_TOKEN", "");
    vi.stubEnv("VERCEL_PROJECT_ID", "prj_without_templates");
