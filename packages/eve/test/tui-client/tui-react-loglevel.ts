...
/**
 * The `tui-loglevel` smoke, run against the React renderer from `dist`: foreign
 * (dev-server) output is captured regardless of mode, so `/loglevel` applies
 * retroactively — hide removes past lines, show restores buffered ones at their
 * original chronological position. Proves React parity for the log-capture +
 * display-mode flow. Serverless (unreachable host), like its terminal twin.
 */
const UNREACHABLE_HOST = "http://127.0.0.1:49217";
const VISIBLE_STDOUT_MARK = "REACT_LOGLEVEL_STDOUT_4f2";
const VISIBLE_STDERR_MARK = "REACT_LOGLEVEL_STDERR_8d5";
const HIDDEN_STDOUT_MARK = "REACT_LOGLEVEL_HIDDEN_1a9";
...
void (async () => {
  ...
  try {
    ...
    if (!ordered) {
      throw new Error(`restored logs should keep chronological order (${positions.join(", ")})`);
    }

    input.ctrlC();
    await runPromise;
    process.stdout.write("[tui-react-loglevel] React parity for buffered /loglevel verified\n");
  } catch (error) {
    process.exitCode = 1;
    renderer.shutdown(); // restore stdout/stderr so the failure is visible
    process.stdout.write(`\n[tui] tui-react-loglevel smoke test failed: ${String(error)}\n`);
  }
})();
