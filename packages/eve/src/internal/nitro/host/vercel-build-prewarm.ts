 ...
 * break at runtime.
 *
 * Returns `true` after validating a deployable Vercel build's template
 * requirements, `false` outside a deployment build.
 */
...
function hasVercelDeploymentContext(): boolean {
  return Boolean(process.env.NOW_BUILDER?.trim() || process.env.VERCEL_PROJECT_ID?.trim());
}
