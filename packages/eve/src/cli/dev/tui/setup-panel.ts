...
function renderStatusText(status: FlowPanelStatus, theme: Theme): string {
  ...
  const emphasisStart = status.text.indexOf(status.emphasis);
  if (emphasisStart === -1) return theme.colors.dim(status.text);
  const emphasisEnd = emphasisStart + status.emphasis.length;
  return [
    theme.colors.dim(status.text.slice(0, emphasisStart)),
    theme.colors.yellow(status.text.slice(emphasisStart, emphasisEnd)),
    theme.colors.dim(status.text.slice(emphasisEnd)),
  ].join("");
}

/** Shared status body used by setup and external-action panels. */
export function renderFlowPanelStatus(status: FlowPanelStatus, theme: Theme): string {
