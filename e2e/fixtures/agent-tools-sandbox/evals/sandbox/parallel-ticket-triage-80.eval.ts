...
const FILE_PATH = "/workspace/ticket-triage-80.tsv";
const TICKETS = Array.from(
  { length: 80 },
  (_, index) => `TCK-${String(5001 + index).padStart(4, "0")}`,
);
...
export default defineEval({
  description:
    "Sandbox measurement: natural ticket-triage request over 80 independent support-ticket lookups.",
  tags: ["parallel-benchmark"],
  timeoutMs: 300_000,
  async test(t) {
    const turn = await t.send(
      [
        "I need a launch-support triage table for this larger ticket queue.",
    ...
    t.log(
      measurementLog({ config: MEASUREMENT, events: turn.events, scenario: "ticket-triage-80" }),
    );
    t.succeeded();
