...
HITL renders as Slack buttons and selects. Each prompt is bound to the verified Slack actor who caused it, independently of any custom session auth returned by `onAppMention` or `onDirectMessage`. A click from another user is rejected privately and does not resume the parked session.
