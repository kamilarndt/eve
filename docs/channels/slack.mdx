...
HITL renders as Slack buttons and selects in the thread. By default, only the Slack user who initiated the current turn can use eve's controls; other users receive a short ephemeral explanation and their response is ignored. Custom or previously posted controls without an eve responder binding retain their existing behavior. The responder is derived from the current caller's Slack `user_id`, so each new turn can have a different responder. When a turn has no Slack user associated with its caller, eve posts the prompt without actionable controls.

When the allowed user responds, the parked session (paused awaiting input) resumes.
