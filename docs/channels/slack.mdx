...
HITL renders as Slack buttons and selects. Each prompt is bound to the authenticated Slack user who caused it. A click from another user is rejected privately and does not resume the parked session. If you override inbound auth while keeping the default `input.requested` handler, preserve the verified Slack user id as the `user_id` auth attribute.
