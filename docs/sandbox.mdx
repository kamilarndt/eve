...
The sandbox is the agent's isolated bash environment: a filesystem rooted at `/workspace` where it can run shell commands, execute scripts, and read or write files without ever touching your app runtime. Every eve agent has exactly one. The built-in `bash`, `read_file`, `write_file`, `download_file`, `glob`, and `grep` tools already target it, and your authored code can too.
...
| `download_file`            | expose a file up to 1 MiB as a browser download |
...
`download_file` carries the file as a data URL through the authenticated session event stream, so it works with every sandbox backend and survives sandbox suspension. The 1 MiB cap is deliberate: larger files would bloat durable workflow history. Upload larger artifacts to object storage from an authored tool and return its download URL instead.
