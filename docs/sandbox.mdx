...
With the Vercel backend, attach authorization directly to the route that needs it and choose an explicit credential resolution mode:
...
`auth` uses the same authorization contract as connections and authored tools: a static `getToken`, a per-user strategy, or an interactive strategy such as Vercel Connect. When any rule has `auth`, the backend-level `credentialResolution` is required. A rule may override it. Native Vercel `match` filters remain available on both authenticated and unauthenticated rules.
...
Eve removes resolved tokens after every step. Unresolved eager routes remain closed, while unresolved on-request routes revert to their trigger rules. Managed policies reject authored `forwardURL` rules and reject policy replacement through `onSession({ use })` or `sandbox.setNetworkPolicy()`; a second policy writer would otherwise remove Eve's transforms and triggers. Brokered credentials are unavailable during template bootstrap.
