...
The string passed to `connect()` is the concrete connector UID, not the bare service host. The curated defaults are `mcp.linear.app/linear` for Linear and `mcp.notion.com/notion` for Notion. `/connect` first attaches that UID. If it is unavailable, you explicitly find an existing connector that supports user authorization or create a named one; after attachment succeeds, eve writes that exact UID into this file. Connect-managed OAuth is user-scoped by default, so the runtime resolves the active user's token before each tool call. If Connect requires authorization, eve emits `authorization.required`, parks the turn, and resumes it after the user finishes the browser flow.
