...
Use the terminal UI for the fastest local feedback loop. `eve dev` boots the runtime, lets you chat
with the agent, shows model and tool progress, and keeps approvals and questions in the same
terminal. Use `eve dev --no-ui` when another client or an automated check will drive the server.
...
- If a tool needs an authorized [connection](../connect/connections), the URL shows up right in the transcript, and the turn picks back up once you finish the flow.
