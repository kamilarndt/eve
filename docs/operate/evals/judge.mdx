...
Use an LLM judge only when no deterministic [assertion](./assertions) captures what “good” means,
such as factual correctness or summary quality. Judges add cost, latency, and another model's
variance to the result, so they are a poor substitute for a crisp behavioral contract.

The `t.judge.*` assertions are the only model-backed ones. They resolve a judge separately from the
agent under test; eve uses that model for scoring, never to replace the tested agent.
