...
Run evals locally while you iterate; point the same files at an immutable deployment URL when you
need an end-to-end production smoke test. `eve eval` discovers `.eval.ts` files under `evals/`,
boots a local dev server unless you supply `--url`, runs cases concurrently, and prints a summary.
