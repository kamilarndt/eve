...
Keep one behavior in each eval until repetition makes a dataset useful. A file is one graded case by
default; it can fan out over a dataset by default-exporting an array. The runner executes each
`test(t)`, captures every event, and computes a verdict from the [assertions](./assertions) you
recorded.

Single-turn, multi-turn, human-in-the-loop (HITL), and dataset cases all use one shape: an
`async test(t)` function that drives the agent and asserts inline.
