...
Use the local target for fast iteration and `--url` to verify a real deployment. An eval target is
always an HTTP URL: `eve eval` starts a local dev server, while `eve eval --url <url>` uses an
existing server. The same files work against both, which lets CI reuse behavior checks as
end-to-end tests.
...
- `t.target.dispatchSchedule(id)` triggers a [schedule](../../build/schedules) through the dev-only schedule route and returns the session ids it created. It works only against a target with dev routes enabled (the local `eve eval` dev server, or a deployment running in development mode), and throws otherwise.
