...
Choose the cheapest assertion that captures the behavior you care about. Check events, tool calls,
schemas, and exact values before reaching for fuzzy similarity or a model judge. Each assertion
records a result on `t`, which lets one run report every failed expectation instead of stopping at
the first one.

There are two deterministic forms: run-level methods on `t`, and `t.check` for a specific value.
For model-graded assertions, see [Judge](./judge).
...
`t.completed()` subsumes `t.didNotFail()`, so reach for `completed` unless you specifically want to allow a parked run. The structured output that `t.outputEquals` and `t.outputMatches` read is the agent's structured output (see the [output schema guide](../../connect/typescript-client/output-schema)).
