...
You do not need a reporter to run evals. eve grades them and prints a console summary itself.
Reporters from `eve/evals/reporters` add external destinations such as Braintrust or JUnit output.

> **Security consequence:** Eval artifacts can contain prompts, model output, tool data, and user
> identifiers. Make sure an external reporter is approved for that data before enabling it.
