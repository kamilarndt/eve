...
Start with one rule: model-directed work is untrusted, even when the model is behaving as intended.
Keep credentials and application code in the eve server process; give shell commands and working
files only the isolated sandbox access they need. The two contexts below make that separation
concrete.
...
The app runtime is the trusted side. Your tool implementations, model calls, connections, state,
and durable execution all run here, with `process.env` and full Node.js available. (On Vercel, this
is a Vercel Function.) A model can still ask a tool to act, so trusted location does not remove the
need for input validation, authorization, approval, and least-privilege credentials.
...
Credential brokering gives the model _authenticated_ network access from inside the sandbox, like a `git clone` of a private repo or an authenticated `curl`, when there's no [tool](../build/tools) or [connection](../connect/connections) to route it through. On the Vercel Sandbox backend, auth headers get injected at the sandbox's network firewall for matching domains. The secret stays in the app runtime; the sandbox process only ever sees the response. See [Vercel Sandbox Credential Brokering](https://vercel.com/docs/sandbox/concepts/firewall#credentials-brokering) for the platform mechanism, and [Sandbox](../build/sandbox) for the eve policy API.

## Connection credentials

[Connection](../connect/connections) tokens (MCP and OpenAPI) come from either `getToken()` or an interactive OAuth flow, and eve injects the resolved token into every outbound request. The token is cached per step and never serialized to durable state.

## Channel verification

A [channel](../connect/channels) is your agent's front door, so authenticating inbound traffic is its job. The built-in platform channels follow two rules, and so must any channel you write yourself:
