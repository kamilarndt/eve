...
You provide the agent's instructions and capabilities; eve owns the model/tool cycle. We keep that
loop in the framework so durability, streaming, tool results, and resumption behave the same across
applications. Extend it with tools, skills, connections, and subagents rather than rebuilding the
driver.

The default loop also includes the built-in tools below. Durability and resumption are described in
[Execution and Durability](./execution-and-durability).

## Compaction

The loop keeps a long session from overflowing the model's context window. Once the conversation crosses a fraction of the window (`thresholdPercent`, `0.9` by default), it summarizes older turns and keeps going. Tune it under [`compaction`](../build/agent-config#compaction) in `agent.ts`:
...
These ship with every agent, no imports. The harness shows the model the tool descriptors first, then executes only what the model actually calls; discovery never runs them. The shell and file tools (`bash`, `read_file`, `write_file`, `glob`, `grep`) live in the app runtime and proxy their work into the agent's single [sandbox](../build/sandbox); the rest run in the app runtime. The "Where it runs" column below names where each tool's effect lands.
...
| `ask_question`      | Ask the user a clarifying question or choice and park until they answer.                                                                                        | App runtime   |
| `agent`             | Delegate a subtask to a copy of itself (shares the parent sandbox + tools, fresh history/state).                                                                | App runtime   |
| `load_skill`        | Pull an on-demand [skill](../build/skills)'s instructions into the current turn. Present only when the agent declares skills.                                   | App runtime   |
| `connection_search` | Discover tools across declared [connections](../connect/connections); matched tools become directly callable. Present only when the agent declares connections. | App runtime   |

Notes:

- **`agent`** runs a copy of the current agent on a focused task. It inherits the same tools, connections, and instructions, but starts with fresh conversation history and fresh [state](../build/state). The child shares the parent's sandbox filesystem. See [Subagents](../build/subagents).
- **`load_skill`** only pulls instructions into context. It adds no executable capability, because behavior still comes from the tools the agent already has.
- **`connection_search`** makes a connection's tools callable by their qualified name (e.g. `linear__list_issues`). It's registered only when the agent has connections.
- **`web_search`** has no local executor; the provider runs it. To supply your own implementation, override it with `defineTool()`.

> **Security consequence:** Review the built-in tools before production use. Disable, wrap,
> restrict, or require approval for any tool that can access the filesystem, network, shell, or
> sensitive data.
...
- **Author a new tool** when you want a capability the harness does not ship. Give it a fresh slug under `agent/tools/` and it joins the built-ins instead of replacing one. See [Tools](../build/tools) for the authoring model.
...
With it on, the model can orchestrate subagents from model-authored JavaScript as one durable step. See [Model-Authored Workflows (Experimental)](../build/advanced/model-authored-workflows).
