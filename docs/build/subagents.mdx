...
Do not split an agent merely to make the tree look modular. Use a subagent when work benefits from
fresh context, parallel execution, a narrower set of tools, or a genuinely different specialist
role. If the agent only needs an optional procedure, a [skill](./skills) is simpler.

eve provides two kinds: the built-in `agent` tool, which delegates to a copy of the current agent,
and declared subagents, which are specialists with their own directory.
...
A declared subagent lives under `agent/subagents/<id>/` and uses the same `defineAgent` helper as
the root. Its location under `subagents/` is the only thing that marks it as a subagent. Declare one
when the child needs a clearly different prompt, role, or set of tools.
...
A declared subagent inherits nothing from the root's authored files. We isolate it this way so its
prompt and permissions can be reviewed without tracing implicit inheritance. Discovery treats its
directory as its own agent root, so it has only the instructions, tools, connections, skills,
sandbox, hooks, and nested subagents authored under `agent/subagents/<id>/`. An absent slot falls
back to the framework default, not to the root's version.
...
Split out a subagent when the task needs a different prompt or specialist role, a narrower set of
tools, or its own runtime context. Don't reach for one when a [skill](./skills) would do. If the
agent can keep its identity and needs only an optional procedure, a skill is the lighter choice.
