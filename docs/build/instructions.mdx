...
Put only the agent's permanent identity and standing rules in instructions. eve prepends them to
every model call in the session, so a long situational procedure costs context even when the turn
does not need it. Put that procedure in a [skill](./skills) instead.

Instructions are the always-on system prompt. Use them for behavior that should hold on every turn,
such as a role, tone, or constraint.
...
To resolve the prompt at runtime from session context (auth, tenant, or channel), wrap `defineInstructions` in a `defineDynamic` resolver. See [Dynamic Capabilities (Experimental)](./advanced/dynamic-capabilities).

If your application needs an automated-system disclosure, put the channel-appropriate wording in instructions or the channel response. See [Responsible Use](../operate/responsible-use) for the baseline operational checklist.
