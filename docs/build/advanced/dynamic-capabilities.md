---
title: "Dynamic Capabilities (Advanced)"
...
Most projects should declare tools, skills, and instructions as files that are the same for every
session. They are easier to inspect, test, and authorize. Use `defineDynamic` only when a capability
genuinely depends on the caller, tenant, channel, feature flags, or external data.

> **Recommendation:** Start with the static forms in [Tools](../tools), [Skills](../skills), and
> [Instructions](../instructions). Move one capability to `defineDynamic` when you can name the
> runtime condition that makes a static declaration insufficient.
...
A dynamic tool or skill whose name matches an **authored** one **overrides** it. We allow that case
so a per-caller resolver can intentionally replace a static default. Two **dynamic** resolvers
emitting the same name is ambiguous and throws; namespace one of the keys manually to resolve it.
