...
You probably do not need a remote agent. Start with a [declared local subagent](../subagents) when
both agents can share one application and deployment. Use `defineRemoteAgent` when the specialist
must be independently owned, deployed, scaled, or authorized; it calls that separate eve
deployment as if it were a local subagent.

> **Security consequence:** A remote delegation crosses an HTTP and authorization boundary. The
> child receives the message you send, runs under its own deployment policy, and reports through
...
If you are calling another Vercel-deployed eve agent, we recommend `vercelOidc()`. The remote
verifies the OIDC token to authorize the caller. See
[Authentication](../../operate/authentication) for the receiving side.
...
2. The parent turn parks (suspends durably without holding compute; see [Execution and Durability](../../concepts/execution-and-durability)) until the remote posts a terminal callback.
