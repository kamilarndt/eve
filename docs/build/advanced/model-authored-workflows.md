---
title: "Model-Authored Workflows (Experimental)"
...
Most agents should call subagents directly. Enable the `Workflow` tool only when the model needs to
decide a multi-stage coordination plan at runtime—for example, fan out over a list produced by an
earlier subagent and then combine the results. The tool lets the model write JavaScript that
coordinates its own subagents as one durable step.

> **Current limitation:** This API is experimental and can change in any release. It is the
> agents-only slice of [code mode](../agent-config#fields), the broader `codeMode` flag that routes
> an agent's tools through model-authored JavaScript.
...
Without that file, the `Workflow` tool stays off. We keep it opt-in because model-authored control
flow is harder to predict and evaluate than direct tool calls. It earns its keep only when the
agent has subagents (or the built-in `agent`) worth coordinating:
...
[Code mode](../agent-config#fields) is the broader version, where the model drives _all_ of an agent's tools (files, shell, web, and agents) from JavaScript. A workflow covers only the subagents. The two do not interfere. Enabling the `Workflow` tool leaves code mode untouched, and an agent can run both at once.
