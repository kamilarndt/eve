...
When authored code needs to know which session it is serving—or reach that session's sandbox,
skills, or durable state—use the runtime context eve passes into the callback. Do not capture this
information during module loading; there is no active session then.

The `ctx` parameter is available in tool `execute`, hook handlers, and channel event handlers:
...
These APIs work only while eve is running an authored callback. We fail immediately outside that
managed context rather than returning data from the wrong session.
