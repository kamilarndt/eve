...
Use a hook when you need to observe the agent without giving the model another tool. Hooks subscribe
to runtime stream events for audit logging, metrics, alerting, or copying session data into your own
analytics store. We run them after each event is durably recorded so an observer cannot leave the
event stream half-written. See [Channels](../connect/channels) for platform-specific events.
...
Hooks always run after the event is durably recorded. If a hook throws, the stream stays
consistent, but the turn still fails; observation code is part of the runtime path, not a detached
best-effort worker.
