...
Use a schedule when the application should start work on its own clock: daily digests, data syncs,
cleanup sweeps, or heartbeats. Each schedule is one file under `agent/schedules/` carrying a cron
expression. Schedules are root-only because the root application owns deployment entrypoints and
delivery channels; a subagent does work only when its parent delegates to it.
