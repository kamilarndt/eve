...
Every eve client turn returns two handles because resuming work and reading its event history are
different operations. The TypeScript client tracks both for you:
...
`stream()` attaches to an existing session stream; to send new user input, use `send()`. For overriding the cursor with `startIndex` and the full reconnection model, see [Streaming](./streaming#open-a-stream-manually).
