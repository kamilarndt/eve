...
`ClientSession` sends one turn at a time. Wait for its response stream to settle before sending the
next turn. We keep turns sequential because the next message must build on the model and tool
history recorded by the previous one. A fresh session starts on the first send; later sends
continue it while the previous turn left the session waiting.
