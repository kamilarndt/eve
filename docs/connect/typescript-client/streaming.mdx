...
Every `ClientSession.send()` posts the turn, then reads its NDJSON (newline-delimited JSON) event
stream. Use `result()` when you only need the outcome; iterate the `MessageResponse` when the UI or
job should react while work is running. A response can be consumed only once.
...
For the complete event table, see [Stream Events](../../reference/stream-events).
