...
Pass `outputSchema` when code needs a typed result rather than prose it must parse. eve asks the
model to satisfy the schema before the turn settles and emits the payload as `result.completed`.
The server remains authoritative for validation; a TypeScript generic alone does not validate a
model response.
...
For task-mode output that belongs to the agent or subagent definition itself, see [`agent.ts`](../../build/agent-config#structured-task-output) and [Subagents](../../build/subagents).
