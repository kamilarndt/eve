...
`eve/next` ships a Next.js frontend and an eve agent as a single project. Wrap your config with `withEve()` to run both from one dev server and one Vercel deploy. [`useEveAgent`](./index) finds the mounted routes on its own, so there's no CORS to configure and no URL env vars to keep in sync.

## Prerequisites

- The `eve` package installed in your project (`npm install eve@latest`).
- An existing eve agent directory. If you don't have one, start from [Quickstart](../../quickstart).
- A Next.js app to mount the agent in.

## Wrap the Next.js config

```ts title="next.config.ts" check
...
For a public demo, use `none()` (also from `eve/channels/auth`) to skip authentication. See [Channels](../channels) and [Authentication](../../operate/authentication).
