...
Use this channel when the agent should appear as a native Linear agent, not a comment bot. Users
delegate work through Linear Agent Sessions, eve receives `AgentSessionEvent` webhooks at
`/eve/v1/linear`, and progress returns as native Agent Activities such as `thought`, `action`,
`elicitation`, `response`, and `error`.
...
Install the Linear app in a test workspace, delegate an issue to the agent, and confirm a native Agent Session is created. The session should show an activity while work runs and a response activity at completion. Verify the webhook signature secret, app access token, and Agent Session event subscription when Linear records a failed delivery.
