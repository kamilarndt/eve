...
Build a custom channel only when the HTTP channel or a provider adapter cannot represent your
transport. You will own the parts a built-in adapter normally handles: request verification,
session mapping, continuation tokens, native delivery, retries, and human-input presentation.

Custom channels can expose HTTP or WebSocket endpoints, start or resume sessions, and observe
runtime events before delivering them back to the platform.

> **Security consequence:** A custom route is an internet-facing application route. Verify the raw
> request before trusting its identity or payload, and never treat a continuation token as user
> authentication.
...
The projection is re-evaluated whenever adapter state changes after channel event handlers run. Dynamic tool resolvers read it via `ctx.channel.metadata` and narrow it with `isChannel`. See [Dynamic capabilities](../../build/advanced/dynamic-capabilities) for the full consumption pattern.
