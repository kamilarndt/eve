...
Authenticates anonymous requests only when they are addressed to a loopback hostname (`localhost`, `*.localhost`, `127.0.0.0/8`, or `::1`). Each local TUI resolves the logged-in Vercel CLI user's stable id and registers a grant under `.eve/dev-auth/`; on POSIX systems, eve restricts the server and grant directories to the current OS user. The request carries only the grant's random token. `localDev()` resolves that token on each request and returns a `user` principal, so Connect-backed connections can complete and reuse that user's interactive authorization. When `eve dev --host` binds another interface, only a valid grant receives this local-user path; an anonymous LAN request still falls through.

The grant is bound to one dev-server run and one TUI process. The TUI rechecks the CLI identity before every turn and rotates the grant when the user changes. A TUI that uses the copyable local-server URL registers its own grant. A normal exit revokes the grant; after an abrupt exit, the server rejects and removes it when it observes that the issuing process is gone. Worker reloads do not carry identity state because the mapping lives outside the worker environment. The registry trusts processes running as the same local OS user; it is not a production authentication mechanism.

The anonymous host check keys off the request URL's hostname rather than the bare `process.env.VERCEL` flag, and that's deliberate: a deployment outside Vercel leaves `VERCEL` unset, so sniffing that flag alone would wave through all public traffic. There are two narrow process-level exceptions. An `eve dev` process accepts a non-loopback request only after validating its local-user credential. `vercel dev`, detected by `VERCEL=1` and `VERCEL_ENV=development` together, keeps its existing anonymous development principal even when it serves over a non-loopback host. Every other non-loopback request returns `null` and falls through.
