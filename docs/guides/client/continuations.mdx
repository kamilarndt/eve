...
serializable set of event IDs already exposed by the client. Together these fields make a
`SessionState`.
...
reconnect to the right physical stream position. `seenEventIds` retains event IDs so exact
re-deliveries remain hidden even when Workflow appends them much later. Treat it as opaque client
state and store the full object rather than rebuilding selected fields.
