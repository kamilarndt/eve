...
For an OpenID Connect (OIDC) protected deployment, pass `auth: { oidc: token }` or use `oidc(token)`. The client resolves the token once per request and sends it as both the bearer credential and Vercel's trusted-OIDC header:

```ts
import { Client, oidc } from "eve/client";

const client = new Client({
  host: "https://agent.example.com",
  auth: oidc(async () => await getOidcToken()),
});
```

For Vercel OIDC, use `vercelOidcAuth()` so the client resolves the token through `@vercel/oidc` before each request:

```ts
import { Client, vercelOidcAuth } from "eve/client";
...
`vercelOidcAuth()` accepts `{ team, project, expirationBufferMs }` and forwards those options to `@vercel/oidc`.
