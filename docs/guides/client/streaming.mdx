...
and suppresses an event before it reaches iteration, `result()`, evals, or frontend stores only when
its server-provided event ID was already exposed. Equal payloads with distinct IDs are preserved, and
events without IDs are always exposed. Filtering never infers duplication from payloads, event types,
timestamps, or turn coordinates.

Persist the complete `session.state` after consuming a response. In addition to `streamIndex`, it
contains the exposed event IDs that keep replay filtering correct after a process restart. When a
state has no `seenEventIds`, the client starts with an empty set and exposes every subsequent ID once.
...
`startIndex` is a physical offset. Overriding it to a different position starts a fresh event-ID set
at that position, so the client does not hide requested history based on a cursor from another offset.
