...
Workflow steps may be delivered more than once. eve stamps each durable event with a stable `meta.id`, and `ClientSession` suppresses repeated IDs before they reach your iterator, reducer, or `MessageResult.events`. The session's `streamIndex` still advances over those physical records, so reconnects resume at the correct position.
